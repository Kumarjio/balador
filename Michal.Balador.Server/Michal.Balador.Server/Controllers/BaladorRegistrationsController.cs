...
namespace Michal.Balador.Server.Controllers
{
    ...
    /// </summary>
    [Authorize]
    [RoutePrefix("api/balador/registrations")]
    public class BaladorRegistrationsController : ApiController
    {
        const string  RegistrationLookupAction= "RegistrationBaladrorLookupAction";
        ...
        [Route("{id}", Name = RegistrationLookupAction)]
        [HttpGet]
       ...
       // [ValidateModel]
        ...
        public async Task<IHttpActionResult> DeleteAll()
        {
            ...
            {
                string msg = string.Format(CultureInfo.CurrentCulture, BaladorResource.RegistrationController_DeleteAllFailure, ex.Message);
                Configuration.DependencyResolver.GetLogger().Error(msg, ex);
                HttpResponseMessage error = Request.CreateErrorResponse(HttpStatusCode.InternalServerError, msg, ex);
                return ResponseMessage(error);
            }
        ...
        protected virtual async Task AddPrivateFilters(string user, WebHook webHook)
        {
            ...
            foreach (IWebHookRegistrar registrar in registrars)
            {
                ...
                {
                    string msg = string.Format(CultureInfo.CurrentCulture, BaladorResource.RegistrationController_RegistrarStatusCode, registrar.GetType().Name, typeof(IWebHookRegistrar).Name, rex.Response.StatusCode);
                    Configuration.DependencyResolver.GetLogger().Info(msg);
                    throw;
                }
                catch (Exception ex)
                {
                    string msg = string.Format(CultureInfo.CurrentCulture, BaladorResource.RegistrationController_RegistrarException, registrar.GetType().Name, typeof(IWebHookRegistrar).Name, ex.Message);
                    Configuration.DependencyResolver.GetLogger().Error(msg, ex);
